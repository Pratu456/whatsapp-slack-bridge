  ...
  try {
    ...
    const workspaceCards = tenants.length === 0
      ? `<div style="grid-column:1/-1;text-align:center;padding:48px 24px;background:#0c0c12;border:1px dashed rgba(255,255,255,.08);border-radius:16px">
          ...
          <a href="/auth/slack" style="display:inline-flex;align-items:center;gap:8px;background:#25D366;color:#000;padding:10px 20px;border-radius:10px;font-size:13px;font-weight:700;text-decoration:none">Connect Slack →</a>
        </div>`
      : tenants.map((t, i) => {
  ...
  <div class="content">
    ...
    <div id="tab-overview" class="panel on">
      ...
      <div class="section-hd">
        ...
        ${canAddWorkspace
          ? `<a href="/auth/slack" style="display:inline-flex;align-items:center;gap:6px;background:#25D366;color:#000;padding:8px 16px;border-radius:9px;font-size:12px;font-weight:700;text-decoration:none">+ Add workspace</a>`
          : ""}
      </div>
...
router.post('/send-invite', requireAuth, async (req, res) => {
  try {
    ...
    if (!tenants.rows.length) return res.json({ success: false, error: 'No active workspace found' });
    const tenant = tenants.rows[0];
    const privateNumber = tenant.meta_phone_number_id || process.env.META_PHONE_NUMBER_PRIVATE;
    const groupNumber = tenant.meta_group_phone_number_id || process.env.META_PHONE_NUMBER_GROUP;
