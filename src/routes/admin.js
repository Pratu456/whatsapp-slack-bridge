...
router.get('/', auth, async (req, res) => {
  try {
    ...
    const tenantRows = tenants.map(t => `
      <tr class="tr-hover" onclick="location.href='/admin/tenant/${t.id}?pwd=${pwd}'" style="cursor:pointer">
        <td>
          <div style="display:flex;align-items:center;gap:10px">
            ...
            <div>
              ...
              <div style="font-size:11px;color:rgba(255,255,255,.3)">${t.slack_team_name || 'No workspace'}</div>
            </div>
          </div>
        </td>
        ...
        <td onclick="event.stopPropagation()" style="white-space:nowrap">
          ${!t.is_active
            ? `<button onclick="activate(${t.id})" class="btn-xs btn-xs-green">Activate</button>`
    ...
    const recentMsgRows = recentMessages.rows.length === 0
      ? '<div style="padding:20px 0;color:rgba(255,255,255,.25);font-size:13px;text-align:center">No messages yet</div>'
      : recentMessages.rows.map(m => `
        <div style="display:flex;align-items:center;gap:12px;padding:11px 0;border-bottom:1px solid rgba(255,255,255,.04)">
          <div style="width:30px;height:30px;border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;flex-shrink:0;${m.direction==='inbound'?'background:rgba(37,211,102,.1);color:#25D366':'background:rgba(59,130,246,.1);color:#60a5fa'}">${m.direction==='inbound'?'↓':'↑'}</div>
          <div style="flex:1;min-width:0">
            <div style="font-size:12px;font-weight:600;color:rgba(255,255,255,.8);margin-bottom:2px">${m.company_name} <span style="color:rgba(255,255,255,.3);font-weight:400">· ${m.wa_number}</span></div>
            <div style="font-size:12px;color:rgba(255,255,255,.35);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${m.media_type?'['+m.media_type.split('/')[0]+']':(m.body||'').substring(0,55)}</div>
          </div>
          <div style="font-size:11px;color:rgba(255,255,255,.2);flex-shrink:0">${new Date(m.created_at).toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'})}</div>
        </div>`).join('');
...
:root{
  --g:#25D366;--gd:#1aad52;
  --bg:#060608;--bg1:#0c0c12;--bg2:#111118;--bg3:#16161f;--bg4:#1c1c28;
...
.row2-eq{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px}
...
.modal-box{background:var(--bg2);border:1px solid var(--b2);border-radius:20px;padding:32px;width:100%;max-width:440px;box-shadow:0 32px 80px rgba(0,0,0,.5)}
...
@media(max-width:960px){
  ...
  .main{margin-left:0;padding-top:var(--mob-topbar)}
  .topbar{display:none}
...
@media(max-width:520px){
  .stats-row{grid-template-columns:1fr 1fr}
  .scard-num{font-size:32px;letter-spacing:-1px}
  .scard{padding:16px}
...
<!-- MOBILE TOP BAR -->
<div class="mob-bar">
  <div class="mob-logo"><div class="mob-mark">B</div>BridgeChat</div>
  <div class="mob-right">
    <div class="mob-live"><span class="mob-live-dot"></span>Live</div>
    <button class="hamburger" id="hbg" onclick="toggleDrawer()">
...
<aside class="sidebar">
  <div class="sb-top">
    <div class="sb-logo">
      <div class="sb-mark">B</div>
      <div><div class="sb-name">BridgeChat</div><div class="sb-sub">Admin Panel</div></div>
    </div>
  </div>
  <nav class="sb-nav">
  ...
  <div class="content">
    ...
    <div id="p-dashboard" class="panel on">
      <div class="stats-row">
        <div class="scard">
          <div class="scard-top">
            <div class="scard-icon" style="background:rgba(37,211,102,.1)">🏢</div>
            <div class="scard-trend" style="background:rgba(37,211,102,.1);color:#4ade80">${active} live</div>
          </div>
          <div class="scard-num">${tenants.length}</div>
          <div class="scard-label">Total companies</div>
        ...
        <div class="scard">
          <div class="scard-top">
            <div class="scard-icon" style="background:rgba(59,130,246,.1)">✅</div>
            <div class="scard-trend" style="background:rgba(59,130,246,.1);color:#60a5fa">Active</div>
          </div>
          <div class="scard-num" style="color:#60a5fa">${active}</div>
          <div class="scard-label">Active companies</div>
        ...
        <div class="scard">
          <div class="scard-top">
            <div class="scard-icon" style="background:rgba(139,92,246,.1)">💬</div>
            <div class="scard-trend" style="background:rgba(139,92,246,.1);color:#a78bfa">Today</div>
          </div>
          <div class="scard-num" style="color:#a78bfa">${todayMessages}</div>
          <div class="scard-label">Messages today</div>
        ...
        <div class="scard">
          <div class="scard-top">
            <div class="scard-icon" style="background:rgba(245,158,11,.1)">👥</div>
            <div class="scard-trend" style="background:rgba(245,158,11,.1);color:#fbbf24">All time</div>
          </div>
          <div class="scard-num" style="color:#fbbf24">${totalContacts}</div>
      ...
      <div class="row2">
        <div class="card">
          <div class="card-hd">
            <div class="card-hd-left">
              <div class="card-hd-icon" style="background:rgba(37,211,102,.1)">📈</div>
              <div><div class="card-hd-title">Message activity</div><div class="card-hd-sub">Last 7 days</div></div>
            </div>
          </div>
        ...
        <div class="card">
          <div class="card-hd">
            <div class="card-hd-left">
              <div class="card-hd-icon" style="background:rgba(245,158,11,.1)">⚠️</div>
              <div><div class="card-hd-title">Inactive companies</div><div class="card-hd-sub">No messages in 14 days</div></div>
            </div>
          </div>
          ${inactiveRows}
        </div>
      ...
      <div class="row2-eq">
        <div class="card">
          <div class="card-hd">
            <div class="card-hd-left">
              <div class="card-hd-icon" style="background:rgba(59,130,246,.1)">🔄</div>
              <div><div class="card-hd-title">Recent messages</div><div class="card-hd-sub">Last 8 across all companies</div></div>
            </div>
          </div>
          ${recentMsgRows}
        </div>
        <div class="card">
          <div class="card-hd">
            <div class="card-hd-left">
              <div class="card-hd-icon" style="background:rgba(139,92,246,.1)">🏢</div>
              <div><div class="card-hd-title">Company overview</div><div class="card-hd-sub">Quick status</div></div>
            </div>
          ...
          ${tenants.slice(0,5).map(t=>`
            <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 0;border-bottom:1px solid rgba(255,255,255,.04)">
              <div style="display:flex;align-items:center;gap:10px">
                <div style="width:28px;height:28px;border-radius:7px;background:rgba(37,211,102,.1);border:1px solid rgba(37,211,102,.15);display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:800;color:#25D366;flex-shrink:0">${t.company_name.charAt(0)}</div>
                <div>
          ...
          ${tenants.length>5?`<div style="text-align:center;margin-top:14px"><button class="btn-ghost" onclick="show('companies',document.getElementById('desk-link-companies'))">+${tenants.length-5} more</button></div>`:''}
        </div>
      </div>
    </div>
    ...
    <div id="p-add" class="panel">
      <div style="max-width:680px">
        ...
        <div style="font-size:13px;color:var(--t4);margin-bottom:24px">Manually add a company to the platform</div>
        <div class="card">
          <div class="form-grid">
            <div class="fg"><label>Company name</label><input type="text" id="nCo" placeholder="e.g. Acme Corp"/></div>
            <div class="fg"><label>Claim code</label><input type="text" id="nCode" placeholder="e.g. acme (one word)"/></div>
...
<div class="modal" id="actModal">
  <div class="modal-box">
    ...
    <div class="modal-sub">Assign a Twilio number and claim code to go live.</div>
    <div class="fg" style="margin-bottom:12px"><label>Twilio WhatsApp number</label><input type="text" id="mTwilio" placeholder="+14155238886"/></div>
    <div class="fg"><label>Claim code</label><input type="text" id="mCode" placeholder="e.g. flo (one word)"/></div>
    <input type="hidden" id="mId"/>
    <div class="modal-btns">
      <button class="mbtn-cancel" onclick="closeModal()">Cancel</button>
      <button class="mbtn-confirm" onclick="confirmActivate()">Activate →</button>
    </div>
  </div>
</div>
...
// Navigation — syncs both desktop sidebar and mobile drawer
...
if(document.getElementById('actChart')){
  new Chart(document.getElementById('actChart'),{
    type:'bar',
    data:{labels,datasets:[{data:counts,backgroundColor:'rgba(37,211,102,.12)',borderColor:'#25D366',borderWidth:2,borderRadius:8,hoverBackgroundColor:'rgba(37,211,102,.2)'}]},
    options:{
      responsive:true,maintainAspectRatio:true,
      plugins:{legend:{display:false},tooltip:{backgroundColor:'#111118',borderColor:'rgba(37,211,102,.2)',borderWidth:1,titleColor:'#fff',bodyColor:'rgba(255,255,255,.6)',padding:10,cornerRadius:8}},
      scales:{
        y:{beginAtZero:true,ticks:{color:'rgba(255,255,255,.3)',stepSize:1,font:{size:11}},grid:{color:'rgba(255,255,255,.04)'},border:{display:false}},
        x:{ticks:{color:'rgba(255,255,255,.3)',font:{size:11}},grid:{display:false},border:{display:false}}
      }
    }
  });
}
...
async function loadMessages(){
  try{
    ...
    if(!d.messages||!d.messages.length){
      document.getElementById('msg-content').innerHTML='<div style="text-align:center;padding:48px 24px;color:rgba(255,255,255,.25);font-size:13px">No messages yet</div>';
      return;
    }
    document.getElementById('msg-content').innerHTML=\`<div class="tbl-wrap"><table>
      <thead><tr><th>Time</th><th>Company</th><th>Number</th><th>Direction</th><th>Message</th></tr></thead>
      <tbody>\${d.messages.map(m=>\`<tr class="tr-hover">
        ...
        <td style="font-weight:600;font-size:13px">\${m.company_name}</td>
        <td style="font-size:12px;color:rgba(255,255,255,.4)">\${m.wa_number}</td>
        <td>\${m.direction==='inbound'?'<span class="badge-green">↓ In</span>':'<span style="background:rgba(59,130,246,.1);color:#60a5fa;padding:4px 10px;border-radius:100px;font-size:11px;font-weight:700;border:1px solid rgba(59,130,246,.2)">↑ Out</span>'}</td>
        <td style="font-size:12px;color:rgba(255,255,255,.5);max-width:280px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">\${m.media_type?'['+m.media_type.split('/')[0]+']':(m.body||'').substring(0,80)}</td>
      </tr>\`).join('')}</tbody></table></div>\`;
  }catch(e){
    document.getElementById('msg-content').innerHTML='<div style="color:#f87171;font-size:13px">Error loading messages</div>';
  }
}
...
async function confirmActivate(){
  ...
  const code=document.getElementById('mCode').value.trim().toLowerCase();
  if(!twilio||!code){alert('Please fill in both fields');return}
  const r=await fetch('/admin/activate?pwd='+pwd,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id,twilio_number:twilio,claim_code:code})});
  const d=await r.json();
  if(d.success){closeModal();location.reload()}else alert('Error: '+d.error);
}
...
async function addTenant(){
  ...
  const code=document.getElementById('nCode').value.trim().toLowerCase();
  const tw=document.getElementById('nTwilio').value.trim();
  const tok=document.getElementById('nToken').value.trim();
  if(!co||!code||!tw||!tok){alert('Please fill in all fields');return}
  const r=await fetch('/admin/add?pwd='+pwd,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({company:co,twilio_number:tw,slack_bot_token:tok,claim_code:code})});
  const d=await r.json();
  if(d.success)location.reload();else alert('Error: '+d.error);
}
...
<meta name="viewport" content="width=device-width,initial-scale=1"/>
<title>${t.company_name} — BridgeChat</title>
...
th{padding:10px 14px;text-align:left;font-size:11px;font-weight:700;color:rgba(255,255,255,.3);text-transform:uppercase;letter-spacing:.8px;background:rgba(255,255,255,.03)}
...
router.post('/activate', auth, async (req, res) => {
  try {
    ...
    await pool.query('UPDATE tenants SET is_active = TRUE, twilio_number = $1, claim_code = $2 WHERE id = $3', [twilio_number, claim_code.toLowerCase().trim(), id]);
